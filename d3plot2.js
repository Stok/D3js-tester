...
//</script>
//
var currentNumberOfChannels = 0;
var data = [];

function appendToExistingData(newData, currentNumberOfChannels)
{
	var stringified = JSON.stringify(newData);
	stringified = stringified.replace(['"y_val" : "y_val"', currentNumberOfChannels.toString()].join());
	data.push(stringified);
}


function plotData(svgId, newdata) { 

//d3.select("svg > *").remove();
data = appendToExistingData(newdata, currentNumberOfChannels);
...
//var parsex_val = d3.time.format("%Y%m%d").parse;
...
// Hover line 
  ...
  function mousemove() { 
      ...
      hoverx_val.text(graph_x); // scale mouse position to xScale to show x_val
